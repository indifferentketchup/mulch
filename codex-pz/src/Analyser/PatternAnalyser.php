...
class PatternAnalyser extends Analyser
{
    ...
    public function analyse(): AnalysisInterface
    {
        ...
        foreach ($this->log as $entry) {
            if (!$this->shouldAnalyseEntry($entry)) {
                continue;
            }
    ...
    /**
     * Return true if this entry should be analysed. Subclasses may override to
     * exclude specific entry shapes (e.g. exception entries owned by another analyser).
     * Default always returns true (backward-compatible for all non-PZ usages).
     *
